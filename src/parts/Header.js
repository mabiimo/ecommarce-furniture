...
export default function Header({ theme, position }) {
  ...
  const [isCartChangeed, setCartChangeed] = useState(false);
  ...
  useLayoutEffect(() => {
    if (prevCart.current !== state.cart) {
      ...
      setCartChangeed(true);
      setTimeout(() => {
        setCartChangeed(false);
      }, 550);
    }
  }, [state.cart]);
              ...
              <li className="ml-6">
                <Link
                  className={[
                    ...
                    theme === "white" ? "text-black md:text-white" : "text-white md:text-black",
                    state.cart && Object.keys(state.cart).length > 0 ? "cart-field" : "",
                    isCartChangeed ? "animate-bounce" : "",
                  ].join(" ")}
                  to="/cart"
                >
                  <IconCart />
