    ...
    .map((_, index) => {
      return (
        <div className="px-4 relative card group" key={index}>
          <div className="rounded-xl bg-gray-300 overflow-hidden card-shadow relative" style={{ width: "287px", height: "386px" }}></div>
...
export default function JustArrived() {
  const { data, /*status,*/ error, run, isLoading } = useAsync({ data: { username: "Test" } });
                ...
                return (
                  <div className="px-4 relative card group" key={item.id}>
                    <div className="rounded-xl overflow-hidden card-shadow relative" style={{ width: "287px", height: "386px" }}>
                      <div className="absolute opacity-0 group-hover:opacity-100 transition duration-200 flex items-center justify-center w-full h-full bg-grey bg-opacity-35">
