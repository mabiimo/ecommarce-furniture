...
export default function BrowseRoom() {
  ...
  const ratioClassNames = {
    wrapper: {
      default: {
        "1/9": "col-span-9 row-span-4",
      },
      md: {
        "1/4": "md: col-span-4 md: row-span-1",
        "2/2": "md: col-span-3 md: row-span-2",
        ...
        <div className="grid grid-rows-2 grid-cols-9 gap-4">
          {isLoading ? (
            ...
            data.data.map((item, index) => {
              return (
                <div key={item.id} className={`relative card ${ratioClassNames?.wrapper.default?.[item.ratio.default]} ${ratioClassNames?.wrapper.md?.[item.ratio.md]}`} style={{ height: index === 0 ? 180 : "auto" }}>
